...
export const glossary = [
	...
	// --- Кэши процессора и когерентность ---
	{ term: 'LLC', full: 'Last Level Cache', def: 'кэш последнего уровня (обычно L3); общий для нескольких ядер сокета или чиплета, часто нарезан на срезы (slices) с доступом через кольцевую шину или mesh.' },
	{ term: 'MESI', full: 'Modified, Exclusive, Shared, Invalid', def: 'базовый протокол когерентности кэшей по четырём состояниям строки; гарантирует, что чтение всегда возвращает последнее записанное значение даже при копиях строки в кэшах разных ядер.' },
	{ term: 'MOESI', full: 'Modified, Owned, Exclusive, Shared, Invalid', def: 'расширение MESI пятым состоянием Owned (грязная разделяемая строка) у AMD; владелец отдаёт данные читателям напрямую без немедленного write-back в память.' },
	{ term: 'MESIF', full: 'Modified, Exclusive, Shared, Invalid, Forward', def: 'расширение MESI состоянием Forward у Intel (QPI/UPI); из всех держателей чистой разделяемой строки ровно один назначен Forward и отвечает на запросы — одна cache-to-cache передача вместо лавины ответов.' },
	{ term: 'VIPT', full: 'Virtually Indexed, Physically Tagged', def: 'схема индексации кэша (L1 на x86): набор выбирается по виртуальному адресу параллельно с трансляцией в TLB, а tag — физический; требует, чтобы размер way не превышал размер страницы.' },
	{ term: 'PIPT', full: 'Physically Indexed, Physically Tagged', def: 'схема индексации кэша (L2/L3 на x86): и index, и tag берутся из физического адреса; нет алиасинга и проще когерентность, но индексация ждёт завершения трансляции в TLB.' },
	{ term: 'RFO', full: 'Read For Ownership', def: 'транзакция когерентности: перед записью ядро читает строку с правом владения, инвалидируя её копии в кэшах других ядер (они уходят в Invalid), и заканчивает в состоянии Modified.' },
	{ term: 'SMT', full: 'Simultaneous Multithreading', def: 'одновременная многопоточность (у Intel — Hyper-Threading): два и более логических потока на одном физическом ядре делят его ресурсы, включая кэши L1/L2.' },
	{ term: 'RRIP', full: 'Re-Reference Interval Prediction', def: 'семейство политик замещения для кэшей последнего уровня; предсказывает, как скоро строку снова используют, и сопротивляется вытеснению полезных данных потоком одноразовых обращений (scan-resistance).' },
];
